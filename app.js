...
class TradingDashboard {
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("leverage-select")
  ...
  populateYearFilter() {
    ...
    const years = [
      ...new Set(this.tradingData.map((item) => item.parsedDates.year)),
    ].sort((a, b) => b - a);
  ...
  applyFilters() {
    ...
    this.filteredData = this.tradingData.filter((item) => {
      ...
      if (this.filters.month && item.parsedDates.month !== this.filters.month) {
        return false;
      }
  ...
  updateStats() {
    ...
    // Ваша бизнес-логика CSV: funding < 0 → плачу я; funding > 0 → платят мне.
    // Итоговая комиссия = Σ(fee - funding) = Σfee − Σfunding
    const totalFees = sumFees - sumFunding;
    ...
    if (totalPnlElement) {
      totalPnlElement.textContent = this.formatCurrency
    ...
    if (totalFeesElement) {
      ...
      totalFeesElement.textContent = fmt(totalFees); // со знаком
      totalFeesElement.title = `Σ fee: ${fmt(sumFees)}; Σ funding: ${fmt(
        sumFunding
      )}; total (fee − funding): ${fmt(totalFees)}`;
    }

    // Новая метрика: ГРОСС (до комиссий) = net + totalFees
    // totalPnl у тебя уже NET (после комиссий) → gross = net + fees
    if (totalGrossPnlElement) {
      ...
      totalGrossPnlElement.title = `Net PnL: ${fmt(
        totalPnl
      )} + (fee − funding): ${fmt(totalFees)} = Gross: ${fmt(totalGross)}`;
    }
    console.log(
      "Updated stats - PnL:",
      totalPnl,
  ...
  createPositionCard(position) {
    ...
    const detailCurrency = (label, value) =>
      detailRaw(
        label,
        this.formatCurrency
          ? this.formatCurrency(toNum(value))
          : String(value ?? "—")
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ====== «Умная» липкость фильтров (dock-on-scroll) ====== */
  try {
    const filtersEl = document.getElementById("filters");
    const heroEl =
    ...
    if (filtersEl && heroEl) {
      const io = new IntersectionObserver(
        ([entry]) => {
          ...
          filtersEl.classList.toggle("filters--stuck", stuck);
        },
        { threshold: [0, 1] }
      );
      io.observe(heroEl);
